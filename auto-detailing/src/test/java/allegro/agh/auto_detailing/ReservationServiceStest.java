...
class ReservationServiceTest {
  ...
  @Mock private ReservationSqlService reservationSqlService;

  @InjectMocks private ReservationService reservationService;

  @Test
  void shouldReturnUserReservations() {
    List<ReservationSqlRow> mockReservations =
        List.of(
            new ReservationSqlRow(
                1, "Detailing", 200, "Toyota", "Corolla", "2020", "Black", null, null));
    ...
    assertEquals("Detailing", result.content().getFirst().servicesName());
  ...
  void shouldMapReservationSqlRowToReservationDto() {
    ...
    ReservationSqlRow reservationSqlRow =
        new ReservationSqlRow(
            1, "Cleaning", 100, "Toyota", "Corolla", "2020", "Red", startAt, endAt);
  ...
  void shouldGenerateDailyTimeSlots() {
    ...
    List<LocalTime> expectedTimeSlots =
        List.of(
            LocalTime.of(9, 0),
            LocalTime.of(9, 30),
            LocalTime.of(10, 0),
    ...
    assertEquals(LocalTime.of(9, 0), result.getFirst());
    assertEquals(LocalTime.of(18, 30), result.getLast());
  ...
  void shouldGenerateTimeSlotsForRange() {
    ...
    Map<LocalDate, List<LocalTime>> result =
        ReservationService.generateTimeSlotsForRange(startDate, 2);
  ...
  void shouldFilterSlots() {
    ...
    List<ReservationStartEndTimes> reservations =
        List.of(
            new ReservationStartEndTimes(
                LocalDate.of(2025, 1, 27).atTime(10, 0), LocalDate.of(2025, 1, 27).atTime(12, 0)));
    ...
    availableSlots.put(LocalDate.of(2025, 1, 27), List.of(LocalTime.of(9, 0), LocalTime.of(11, 0)));

    when(reservationService.filterSlots(slots, reservations, 2, 0)).thenReturn(availableSlots);

    Map<LocalDate, List<LocalTime>> result =
        reservationService.filterSlots(slots, reservations, 2, 0);
